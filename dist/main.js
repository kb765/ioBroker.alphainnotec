...
class AlphainnotecAdapter extends utils.Adapter {
    constructor(options = {}) {
        ...
        this.client = axios_1.default.create({ timeout: 10000 });
    ...
    async poll(ip, pin, dataUrl, wsPort) {
        try {
    ...
    async readMeasurements(ip, pin, dataUrl, wsPort) {
    ...
    async readViaWebSocket(ip, pin, wsPort) {
        const endpoint = `ws://${ip}:${wsPort}`;
        try {
            return await new Promise((resolve, reject) => {
                const ws = new ws_1.default(endpoint, "Lux_WS");
                const xmlMessages = [];
                ...
                const finish = (error) => {
                    ...
                    if (!this.hasMeasurementLikeKeys(parsed.discoveredAny)) {
                        const roots = Array.from(new Set(xmlMessages
                            .map((msg) => msg.match(/<([A-Za-z0-9_:-]+)(?:\s|>)/)?.[1]?.toLowerCase())
                            .filter((tag) => Boolean(tag)))).slice(0, 8);
                        reject(new Error(`Luxtronik websocket connected but no measurement-like values were found. wsMessages=${xmlMessages.length}; roots=${roots.join("|") || "none"}; keys=${Object.keys(parsed.discoveredAny).slice(0, 12).join("|") || "none"}; sample=${this.limitText(parsed.rawPreview, 180)}`));
                        return;
                    }
                ...
                ws.on("message", (data) => {
                    ...
                    if (/<Navigation[\s>]/i.test(xml) && ws.readyState === ws_1.default.OPEN) {
                        const navigationTargets = this.extractNavigationTargets(xml);
                        const preferred = navigationTargets.filter((entry) => /temperaturen|eing[aä]nge|ausg[aä]nge|ablaufzeiten|betriebsstunden|fehlerspeicher|abschaltungen|anlagenstatus|w[aä]rmemenge|glt/i.test(entry.name));
    ...
    parseLuxWebSocketMessages(messages) {
        const discoveredAny = {};
    ...
    pickNumberFromDiscovered(values, hints) {
    ...
    collectStructuredTextValues(text, target) {
    ...
    collectNumericObjectValues(source, target, prefix = "") {
        for (const [rawKey, value] of Object.entries(source)) {
            ...
            if (value && typeof value === "object") {
                this.collectNumericObjectValues(value, target, key);
            }
        }
    }
    collectNumericTextValues(text, target) {
    ...
    looksLikeLibrarySource(url, content) {
    ...
    toAbsoluteUrl(baseUrl, inputUrl) {
    ...
    async syncDynamicLuxStates(discoveredData) {
    ...
    getStateRole(section, itemName, valueType) {
    ...
    getStateUnit(section, itemName, value) {
...
if (require.main !== module) {
    module.exports = (options) => new AlphainnotecAdapter(options);
}
else {
